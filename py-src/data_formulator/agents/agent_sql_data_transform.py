...
# 简单的 SQL 表统计缓存（进程级），用于收敛探索/报表等流程中的重复统计计算
SQL_TABLE_STATS_CACHE = {}
MAX_SQL_STATS_CACHE_SIZE = 256


def _sql_stats_cache_key(conn, table_name: str, row_sample_size: int, field_sample_size: int, max_val_chars: int):
    """基于连接对象和统计参数构造缓存 key。"""
    return (
        id(conn),
        table_name,
        int(row_sample_size),
        int(field_sample_size),
        int(max_val_chars),
    )
        ...
        for choice in response.choices:
            ...
            if len(query_blocks) > 0:
                ...
                try:
                    ...
                    result = {
                        "status": "ok",  # Successfully executed SQL query and created view
                        "code": query_str,
                        "content": {
...
def get_sql_table_statistics_str(conn, table_name: str, 
    ...
    为了减少重复计算，会对相同连接 + 表名 + 采样参数的结果做进程级缓存。
    """

    table_name = sanitize_table_name(table_name)

    # 命中缓存则直接返回
    cache_key = _sql_stats_cache_key(conn, table_name, row_sample_size, field_sample_size, max_val_chars)
    cached = SQL_TABLE_STATS_CACHE.get(cache_key)
    ...
    if cache_key is not None:
        if len(SQL_TABLE_STATS_CACHE) >= MAX_SQL_STATS_CACHE_SIZE:
            try:
                first_key = next(iter(SQL_TABLE_STATS_CACHE))
                SQL_TABLE_STATS_CACHE.pop(first_key, None)
            except StopIteration:
                pass
        SQL_TABLE_STATS_CACHE[cache_key] = table_summary_str
