...
_CLIENT_CACHE: Dict[Tuple[str, str, str, str, str], object] = {}
...
def _client_cache_key(
    ...
    return (
        _normalize_str(endpoint),
        _normalize_str(model),
        _normalize_str(api_key),
        _normalize_str(api_base),
        _normalize_str(api_version),
    )
...
class Client(object):
    ...
    def from_config(cls, model_config: Dict[str, str]):
        ...
        cache_key = _client_cache_key(endpoint, model, api_key, api_base, api_version)
        cached = _CLIENT_CACHE.get(cache_key)
        if cached is not None and isinstance(cached, cls):
            return cached

        client = cls(endpoint, model, api_key, api_base, api_version)
