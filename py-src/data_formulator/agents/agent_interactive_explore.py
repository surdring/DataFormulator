...
9. LIMITS:
    - You MUST output at most 4 questions in total (even if the user does not specify a number).
    - Each question 'text' should be a single concise sentence and no longer than 80 Chinese characters.
    - Each 'goal' should be a short phrase (<16 Chinese characters) capturing the core computation or comparison.

10. LANGUAGE REQUIREMENTS:
    - All natural language content that will be shown to the user (including the 'text' and 'goal' fields) MUST be written in Simplified Chinese (简体中文).
    - JSON field names and the allowed values for 'difficulty' and 'tag' MUST remain in English exactly as specified above.
    - Do NOT output any English explanation or commentary outside of the JSON objects.
...
5. QUESTION GROUP GENERATION: 
    ...
    - you have a budget of generating 4 questions in total (or as directed by the user).
        - you MUST NOT generate more than 4 questions in total across 'breadth_questions' and 'depth_questions' combined (even if the user does not specify a number).
...
7. FORMATTING: 
    ...
    - Overall length limits: keep every question text concise (no more than 80 Chinese characters) and each 'goal' under 16 Chinese characters.

8. LANGUAGE REQUIREMENTS:
    - All natural language content that will be shown to the user (including every question text in 'breadth_questions' and 'depth_questions', and the 'goal' field) MUST be written in Simplified Chinese (简体中文).
    - JSON field names and the allowed values for 'difficulty' and 'focus' MUST remain in English exactly as specified above.
    - Do NOT output any English explanation or commentary outside of the JSON objects.
    ...
    def get_data_summary(self, input_tables):
        if self.db_conn:
            ...
            for table in input_tables:
                ...
                table_summary_str = get_sql_table_statistics_str(self.db_conn, table_name, row_sample_size=3, field_sample_size=5, max_val_chars=80)
                data_summary += f"[TABLE {table_name}]\n\n{table_summary_str}\n\n"
        else:
            data_summary = generate_data_summary(input_tables, include_data_samples=False, field_sample_size=5, max_val_chars=80)
        return data_summary
        ...
        if exploration_thread:
            ...
            for i, table in enumerate(exploration_thread, 1):
                ...
                if isinstance(rows, list) and rows and isinstance(rows[0], dict):
                    ...
                    col_count = 0
                thread_summary += f"{i}. {table_name} (rows: {row_count}, cols: {col_count}): {table_description}\n"
            context += f"\n\n[EXPLORATION THREAD]\n\n{thread_summary}"
