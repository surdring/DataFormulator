...
const textRowsCache = new Map<string, any[][]>();
...
export const createTableFromText = (title: string, text: string, context?: string): DictTable | undefined => {
    ...
    // Strip potential markdown code fences (``` / ```csv) that may wrap CSV content
    // This is common when LLM 返回的表格被包在 ```csv ... ``` 中，会干扰首行表头解析
    const cleanedTextLines = text
        .split(/\r?\n/)
        .filter(line => {
    ...
    const cleanedText = cleanedTextLines.join('\n').trim();

    if (!cleanedText) {
        console.log('Invalid text provided for data after stripping code fences. Could not load.');
        return undefined;
    }
    ...
    for (let i = 0; i < cleanedText.length; i++) {
        if (cleanedText.charAt(i) === '\t') tabNum++;
    ...
    let rows = textRowsCache.get(cleanedText);
    if (!rows) {
        rows = isTabSeparated 
            ? d3.tsvParseRows(cleanedText) 
            : d3.dsvFormat(',').parseRows(cleanedText, (row, index) => {
                // Process each row to ensure proper type handling
                return row;
              });
        textRowsCache.set(cleanedText, rows);
    }
...
export const loadBinaryDataWrapper = async (title: string, arrayBuffer: ArrayBuffer, fileName?: string): Promise<DictTable[]> => {
    try {
        const lowerName = (fileName || title || '').toLowerCase();
