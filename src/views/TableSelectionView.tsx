            ...
            <Box sx={{ flex: 1, overflow: 'auto', p: 2 }}>
                {datasets.map((dataset, i) => {
                    ...
                    let tableComponents = dataset.tables.map((table, j) => {
                        let sampleTable = createTableFromFromObjectArray(table.table_name, table.sample, true);
                        let maxDisplayRows = dataset.tables.length > 1 ? 5 : 9;
                        if (sampleTable.rows.length < maxDisplayRows) {
                            maxDisplayRows = sampleTable.rows.length - 1;
                        }
                        let sampleRows = [
                            ...sampleTable.rows.slice(0,maxDisplayRows), 
                            Object.fromEntries(sampleTable.names.map(n => [n, "..."]))
                        ];
                        let colDefs = sampleTable.names.map(name => { return {
                            id: name, label: name, minWidth: 60, align: undefined, format: (v: any) => v,
                        }})

                        let content = <Paper variant="outlined" key={sampleTable.names.join("-")} sx={{width: 800, maxWidth: '100%', padding: "0px", marginBottom: "8px"}}>
                            ...
                            <Box key={j}>
                                <Typography variant="subtitle2" sx={{ mb: 1, fontSize: 12}} color="text.secondary">
                                    {table.url.split("/").pop()?.split(".")[0]}  {
                                        t('dataset.preview.shape')
                                            .replace('{columns}', String(Object.keys(sampleTable.rows[0]).length))
                                            .replace('{rows}', String(sampleTable.rows.length))
                                    }
                                </Typography>
                                {content}
                            </Box>
                        ...
                        <Box key={i}>
                            <Box sx={{mb: 1, gap: 1, maxWidth: 800, display: "flex", alignItems: "center"}}>
                                <Typography sx={{fontSize: 12}}>
                                    {dataset.description} <Typography variant="caption" sx={{color: "primary.light", fontSize: 10, mx: 0.5}}>{`[${t('dataset.preview.sourcePrefix')}${dataset.source}]`}</Typography> 
                                </Typography>
...
export const DatasetSelectionDialog: React.FC<{ buttonElement: any }> = function DatasetSelectionDialog({ buttonElement }) {
    ...
    return <>
        ...
        <Dialog key="sample-dataset-selection-dialog" onClose={() => {setTableDialogOpen(false)}} 
            ...
            >
                <DialogTitle sx={{display: "flex"}}>{t('dataset.dialog.explore.title')}
                    <IconButton
                        sx={{marginLeft: "auto"}}
                        edge="start"
                        size="small"
                ...
                if (file.type === 'text/csv' || 
                    ...
                    if (file.size > MAX_FILE_SIZE) {
                        ...
                        dispatch(dfActions.addMessages({
                            ...
                            "value": `${t('upload.error.fileTooLarge.prefix')}${file.name}${t('upload.error.fileTooLarge.middle')}${sizeMB}${t('upload.error.fileTooLarge.suffix')}`
                        }));
                            ...
                            try {
                                ...
                                if (tables.length == 0) {
                                    dispatch(dfActions.addMessages({
                                        ...
                                        "type": "error",
                                        "component": "data loader",
                                        "value": `${t('upload.error.excelParse.prefix')}${file.name}${t('upload.error.excelParse.suffix')}`
                                    }));
                                }
                            } catch (error) {
                    ...
                    dispatch(dfActions.addMessages({
                        ...
                        "type": "error",
                        "component": "data loader",
                        "value": `${t('upload.error.unsupportedFormat.prefix')}${file.name}${t('upload.error.unsupportedFormat.suffix')}`
                    }));
                }
            }
        }
            ...
            <Tooltip 
                title={serverConfig.DISABLE_FILE_UPLOAD ? (
                    <Typography sx={{ fontSize: '11px' }}>
                        {t('dataset.upload.tooltip.disabled')}<br />
                        <Link 
...
export const TableURLDialog: React.FC<TableURLDialogProps> = ({ buttonElement, disabled }) => {
    ...
    let dialog = <Dialog key="table-url-dialog" onClose={()=>{setDialogOpen(false)}} open={dialogOpen}
        ...
        >
            <DialogTitle  sx={{display: "flex"}}>{t('dataset.urlDialog.title')}
                <IconButton
                    sx={{marginLeft: "auto"}}
                    edge="start"
                    size="small"
            ...
            <DialogContent sx={{overflowX: "hidden", padding: 2, display: "flex", flexDirection: "column"}} dividers>
                <TextField error={tableURL != "" && !hasValidSuffix} autoFocus placeholder={t('dataset.urlDialog.placeholder')} 
                            helperText={hasValidSuffix ? "" : t('dataset.urlDialog.helper.invalid')}
...
export const TableCopyDialogV2: React.FC<TableCopyDialogProps> = ({ buttonElement, disabled }) => {
    ...
    let dialog = <Dialog key="table-selection-dialog" onClose={handleCloseDialog} open={dialogOpen}
        ...
        >
            <DialogTitle  sx={{display: "flex"}}>{t('dataset.pasteDialog.title')}
                <IconButton
                    sx={{marginLeft: "auto"}}
                    edge="start"
                    size="small"
                ...
                <Box sx={{width: '100%',  display:'flex', position: 'relative', overflow: 'auto'}}>
                    ...
                    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                        ...
                        {isOverSizeLimit && (
                            <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: 1, padding: 1, backgroundColor: 'rgba(244, 67, 54, 0.1)', borderRadius: 1, border: '1px solid rgba(244, 67, 54, 0.3)' }}>
                                <Typography variant="caption" sx={{ flex: 1, color: 'error.main', fontWeight: 500 }}>
                                    {t('dataset.pasteDialog.sizeWarning')
                                        .replace('{mbLimit}', (MAX_CONTENT_SIZE / (1024 * 1024)).toFixed(0))
                                        .replace('{mb}', (new Blob([tableContent]).size / (1024 * 1024)).toFixed(2))}
                                </Typography>
                            </Box>
                        )}
                        ...
                        {isLargeContent && !isOverSizeLimit && (
                            <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: 1, padding: 1, backgroundColor: 'rgba(255, 193, 7, 0.1)', borderRadius: 1 }}>
                                <Typography variant="caption" sx={{ flex: 1 }}>
                                    {t('dataset.pasteDialog.largeContent.preview')
                                        .replace('{kb}', String(Math.round(tableContent.length / 1000)))}
                                </Typography>
                                <Button 
                                    size="small" 
                                    variant="outlined" 
                                ...
                                >
                                    {showFullContent ? t('dataset.pasteDialog.largeContent.showPreview') : t('dataset.pasteDialog.largeContent.showFull')}
                                </Button>
                            </Box>
                        )}
                        ...
                        <TextField 
                            ...
                            placeholder={t('dataset.pasteDialog.placeholder')}
                            onPasteCapture={(e) => {
                            ...
                            autoComplete='off'
                            label={t('dataset.pasteDialog.label')}
                            variant="outlined" 
                            multiline 
                        />
                    </Box>
            ...
            <DialogActions>
                <Button variant="text" sx={{textTransform: 'none'}} size="small" onClick={handleCloseDialog}>{t('common.cancel')}</Button>
