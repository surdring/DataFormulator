...
const LazyDataLoadingChatDialog = React.lazy(() =>
    import('./DataLoadingChat').then((m) => ({ default: m.DataLoadingChatDialog }))
);

const LazyReportView = React.lazy(() =>
...
export const DataFormulatorFC = ({ }) => {
    ...
    const handleLoadExampleSession = (session: ExampleSession) => {
        dispatch(dfActions.addMessages({
            ...
            value: `${t('messages.example.loading')}${session.title}`,
        }));
        ...
        fetch(session.dataFile)
            ...
            .then(savedState => {
                ...
                dispatch(dfActions.addMessages({
                    ...
                    type: 'success',
                    component: 'data formulator',
                    value: `${t('messages.example.loaded')}${session.title}`,
                }));
            })
            .catch(error => {
                ...
                dispatch(dfActions.addMessages({
                    ...
                    type: 'error',
                    component: 'data formulator',
                    value: `${t('messages.example.loadFailed')}${session.title}: ${(error as Error).message}`,
                }));
            });
    };
        ...
        <Box sx={{display: 'flex', flexDirection: 'row', height: '100%'}}>
            ...
            <Box sx={{
                ...
                {viewMode === 'editor' ? (
                    ...
                    <React.Suspense fallback={<Box sx={{ p: 2 }}>{t('common.loading')}</Box>}>
                        <LazyReportView />
                    </React.Suspense>
                )}
            </Box>
...
`
    ...
    let dataUploadRequestBox = <Box sx={{
        ...
        <Box sx={{margin:'auto', pb: '5%', display: "flex", flexDirection: "column", textAlign: "center" }}>
            ...
            <Typography fontSize={24} sx={{color: 'text.secondary'}}>{t('home.subtitle')}</Typography>
            <Box sx={{mt: 4, width: '100%', borderRadius: 8, 
                ...
                <Divider sx={{width: '200px', mx: 'auto', mb: 2, fontSize: '1.2rem', color: 'text.disabled'}}>
                    <Typography sx={{ fontSize: 14, color: 'text.disabled' }}>
                        {t('home.loadData.section')}
                    </Typography>
                </Divider>
                <Typography  variant="h4" sx={{mx: 'auto', width: 1080,  fontSize: 24}}>
                    <React.Suspense fallback={<span>{t('common.loading')}</span>}>
                        <LazyDataLoadingChatDialog buttonElement={<><AutoFixNormalIcon sx={{ mr: 1, verticalAlign: 'middle' }} />{t('home.loadData.messy')}</>} />  
                    </React.Suspense>
                    <Box component="span" sx={{ mx: 2, color: 'text.disabled', fontSize: '0.8em' }}>•</Box>
                    <DatasetSelectionDialog  buttonElement={<><CategoryIcon sx={{ mr: 1, verticalAlign: 'middle' }} />{t('home.loadData.examples')}</>} /> 
                    <Box component="span" sx={{ mx: 2, color: 'text.disabled', fontSize: '0.8em' }}>•</Box>
                    <TableUploadDialog buttonElement={<><FolderOpenIcon sx={{ mr: 1, verticalAlign: 'middle' }} />{t('home.loadData.files')}</>} disabled={false} /> 
                    <Box component="span" sx={{ mx: 2, color: 'text.disabled', fontSize: '0.8em' }}>•</Box>
                    <TableCopyDialogV2 buttonElement={<><ContentPasteIcon sx={{ mr: 1, verticalAlign: 'middle' }} />{t('home.loadData.clipboard')}</>} disabled={false} /> 
                    <Box component="span" sx={{ mx: 2, color: 'text.disabled', fontSize: '0.8em' }}>•</Box>
                    <DBTableSelectionDialog buttonElement={<><CloudQueueIcon sx={{ mr: 1, verticalAlign: 'middle' }} />{t('home.loadData.database')}</>} />
                    {/* <br /> */}
                    ...
                    <Typography variant="body1" color="text.secondary" sx={{ mt: 2, width: '100%' }}>
                        {t('home.loadData.description.prefix')}{' '}
                        <Tooltip title={<Box>{t('home.loadData.tooltip.screenshot.title')}<Box component="img" sx={{ width: '100%', marginTop: '6px' }} alt="" src={exampleImageTable} /></Box>}>
                            <Box component="span" sx={{color: 'secondary.main', cursor: 'help', "&:hover": {textDecoration: 'underline'}}}>{t('home.loadData.screenshots')}</Box>
                        </Tooltip>{' '}
                        和{' '}
                        <Tooltip title={<Box>{t('home.loadData.tooltip.textBlock.title')}<Typography sx={{fontSize: 10, marginTop: '6px'}} component="pre">{exampleMessyText}</Typography></Box>}>
                            <Box component="span" sx={{color: 'secondary.main', cursor: 'help', "&:hover": {textDecoration: 'underline'}}}>{t('home.loadData.textBlocks')}</Box>
                        </Tooltip>{' '}
                        {t('home.loadData.description.suffix')}
                    </Typography> 
                </Typography>
            </Box>
