...
let getChannelDisplay = (channel: Channel) => {
    ...
    if (channel == "x") {
        return t('encoding.channel.xAxis');
    } else if (channel == "y") {
        return t('encoding.channel.yAxis');
    }
    return channel;
}
...
export const EncodingBox: FC<EncodingBoxProps> = function EncodingBox({ channel, chartId, tableId }) {
    ...
    let dataTypeOpt = [
        <FormLabel key={`enc-box-${channel}-data-type-label`} sx={{ fontSize: "inherit" }} id="data-type-option-radio-buttons-group" >{t('encoding.label.dataType')}</FormLabel>,
        <FormControl
            ...
            >
                {radioLabel(getIconFromDtype("auto"), "auto", `dtype-auto`, 40, false, t('encoding.dtype.auto'))}
                {radioLabel(getIconFromDtype("quantitative"), "quantitative", `dtype-quantitative`, 40, false, t('encoding.dtype.quantitative'))}
                {radioLabel(getIconFromDtype("nominal"), "nominal", `dtype-nominal`, 40, false, t('encoding.dtype.nominal'))}
                {radioLabel(getIconFromDtype("temporal"), "temporal", `dtype-temporal`, 40, false, t('encoding.dtype.temporal'))}
            </RadioGroup>
        </FormControl>
    ];

    let stackOpt = (chart.chartType == "bar" || chart.chartType == "area") && (channel == "x" || channel == "y") ? [
        <FormLabel key={`enc-box-${channel}-stack-label`} sx={{ fontSize: "inherit" }} id="normalized-option-radio-buttons-group" >{t('encoding.label.stack')}</FormLabel>,
        <FormControl
            ...
            >
                {radioLabel(t('encoding.stack.default'), "default", `stack-default`)}
                {radioLabel(t('encoding.stack.layered'), "layered", `stack-layered`)}
                {radioLabel(t('encoding.stack.center'), "center", `stack-center`)}
                {radioLabel(t('encoding.stack.normalize'), "normalize", `stack-normalize`)}
            </RadioGroup>
        </FormControl>
    ] : [];
            ...
            .then((data) => {
                ...
                if (data["status"] == "ok") {
                    ...
                    dispatch(dfActions.addMessages({
                        ...
                        "type": "error",
                        "value": t('messages.encoding.autoSortFailed')
                    }));
                ...
                dispatch(dfActions.addMessages({
                    ...
                    "component": "EncodingBox",
                    "type": "error",
                    "value": t('messages.encoding.autoSortServerFailed')
                }));
            });
    }

    let sortByOptions = [
        radioLabel(t('encoding.sort.auto'), "auto", `sort-by-auto`)
    ]
        ...
        if (autoSortInferRunning) {
            ...
            if (autoSortResult != undefined && autoSortResult.length > 0) {
                ...
                let autoSortOptTitle = <Box>
                        <Box>
                            <Typography sx={{fontWeight: 'bold'}} component='span' fontSize='inherit'>{t('encoding.sort.orderLabel')}</Typography> 
    ...
    let sortByOpt = [
        <FormLabel sx={{ fontSize: "inherit" }} key={`enc-box-${channel}-sort-label`} id="sort-option-radio-buttons-group" >{t('encoding.label.sortBy')}</FormLabel>,
        <FormControl
    ...
    let sortOrderOpt = [
        ...
        <FormControl
            ...
            >
                {radioLabel(t('encoding.sortOrder.auto'), "auto", `sort-auto`, 60)}
    ...
    let colorSchemeOpt = channel == "color" ? [
            <FormLabel sx={{ fontSize: "inherit" }} key={`enc-box-${channel}-color-scheme-label`} id="scheme-option-radio-buttons-group">{t('encoding.label.colorScheme')}</FormLabel>,
    ...
    let createConceptInputBox = <Autocomplete
        ...
        renderOption={(props, option) => {
            ...
            if (fieldItem) {
                ...
                return (
                    ...
                    >
                        {renderOption || t('encoding.placeholder.newFieldName')}
                    </Typography>
                );
            }
        }}
        ...
        renderInput={(params) => (
            <TextField
                ...
                variant="standard"
                autoComplete='off'
                placeholder={t('encoding.placeholder.field')}
