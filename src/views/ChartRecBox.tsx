...
const chartIdeasInteractiveCache = new Map<string, any[]>();
const chartIdeasAgentCache = new Map<string, any[]>();

const buildIdeasCacheKey = (
    mode: 'interactive' | 'agent',
    tableId: string,
    selectedTableIds: string[],
    encodingSnapshot?: string
) => {
    return JSON.stringify({
        mode,
        tableId,
        baseTables: [...selectedTableIds].sort(),
        encoding: encodingSnapshot || 'none',
    });
};
                ...
                {tables
                    ...
                    .map((table) => {
                        ...
                        return (
                            ...
                            >
                                {table.displayId}
                                {isRequired && <Typography sx={{ fontSize: '10px', color: 'text.secondary' }}>({t('chart.nlTable.required')})</Typography>}
                            </MenuItem>
                        );
                    })
                }
...
export const ChartRecBox: FC<ChartRecBoxProps> = function ({ tableId, placeHolderChartId, sx }) {
    ...
    const placeholderChart = placeHolderChartId
        ? allCharts.find(c => c.id === placeHolderChartId)
        : undefined;
    const encodingSnapshot = placeholderChart ? JSON.stringify(placeholderChart.encodingMap) : undefined;
    ...
    const getQuestion = (): string => {
        return mode === "agent" ? t('chart.prompt.agent.default') : t('chart.prompt.interactive.default');
    };
    ...
    const getIdeasFromAgent = async (
        ...
        if (!startQuestion && !autoRunFirstIdea && !forceRefresh) {
            cacheKey = buildIdeasCacheKey(mode, currentTable.id, selectedTableIds, encodingSnapshot);
            const cacheMap = mode === 'agent' ? chartIdeasAgentCache : chartIdeasInteractiveCache;
            const cached = cacheMap.get(cacheKey);
            if (cached) {
                if (mode === 'agent') {
                    setAgentIdeas(cached);
                } else {
                    setIdeas(cached);
                }
                setThinkingBuffer("");
        ...
        } else if (!startQuestion && !autoRunFirstIdea && forceRefresh) {
            cacheKey = buildIdeasCacheKey(mode, currentTable.id, selectedTableIds, encodingSnapshot);
            const cacheMap = mode === 'agent' ? chartIdeasAgentCache : chartIdeasInteractiveCache;
        ...
        try {
            ...
            let updateState = (lines: string[]) => {
                ...
                if (mode === "agent") {
                    ...
                    if (cacheKey) {
                        chartIdeasAgentCache.set(cacheKey, newIdeas);
                    }
                } else {
                    let questions = dataBlocks.map(block => ({
                        text: block.text,
                    ...
                    setIdeas(questions);
                    if (cacheKey) {
                        chartIdeasInteractiveCache.set(cacheKey, questions);
                    }
                }
            }
            ...
            dispatch(dfActions.addMessages({
                ...
                "value": t('messages.chart.ideasFailed'),
        ...
        .catch((error) => {
            ...
            if (error.name === 'AbortError') {
                dispatch(dfActions.addMessages({
                    ...
                    "component": "chart builder",
                    "type": "error", 
                    "value": t('messages.chart.explorationTimeout'),
    ...
    const exploreDataFromNL = (initialPlan: string[]) => {
        ...
        const handleCompletion = () => {
            ...
            if (completionResult) {
                ...
                dispatch(dfActions.addMessages({
                    ...
                    "component": "chart builder",
                    "type": "success",
                    "value": t('messages.chart.explorationCompleted')
                }));

                // Clear the prompt after successful exploration
                setPrompt("");
            } else {
                dispatch(dfActions.updateAgentWorkInProgress({actionId: actionId, description: t('messages.chart.agentLost.desc'), status: 'warning', hidden: false}));

                dispatch(dfActions.addMessages({
                    "timestamp": Date.now(),
                    "component": "chart builder",
                    "type": "error",
                    "value": t('messages.chart.agentLost.toast')
                }));
            }
        };
        ...
        .catch((error) => {
            ...
            const errorMessage = error.name === 'AbortError' ? t('messages.chart.explorationTimeout') : `${t('messages.chart.explorationFailed.prefix')}${error.message}`;
            ...
            if (error.name === 'AbortError') {
                ...
                dispatch(dfActions.addMessages({
                    ...
                    "component": "chart builder",
                    "type": "error",
                    "value": `${t('messages.chart.explorationFailed.prefix')}${error.message}`,
                    "detail": error.message
                }));
            }
        });
            ...
            <Card variant='outlined' sx={{ 
                ...
                <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1, alignItems: 'flex-end' }}>
                    <TextField
                        ...
                        value={prompt}
                        label={mode === "agent" ? t('chart.prompt.label.agent') : t('chart.prompt.label.interactive')}
                    ...
                    {<Box sx={{display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 0.5, my: 1}}>
                        <Typography sx={{ fontSize: 10, color: "text.secondary", marginBottom: 0.5 }}>
                            {t('chart.ideas.caption.question')}
                        </Typography>
                        ...
                        <Tooltip title={t('chart.ideas.tooltip.refresh')}>
                            <span>
                                <IconButton
                                    size="medium"
                                    disabled={isFormulating || isLoadingIdeas || !currentTable}
                                    ...
                                    onClick={() => getIdeasFromAgent(mode, undefined, false, true)}
                                >
                                    <RotateRightIcon sx={{ fontSize: 22 }} />
                                </IconButton>
                            </span>
                        </Tooltip>
                    </Box>}
                    ...
                    <Box>
                        {agentIdeas.length > 0 && <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, marginBottom: 1 }}>
                            <Typography sx={{ fontSize: 12, color: "text.secondary", ".MuiSvgIcon-root": { cursor: 'help', transform: 'rotate(90deg)', verticalAlign: 'middle', fontSize: 12} }}>
                                {t('chart.agentIdeas.directionsLabel')} <Tooltip title={t('chart.agentIdeas.tooltip.deepDive')}><MovingIcon /></Tooltip>  <Tooltip title={t('chart.agentIdeas.tooltip.branch')}><CallSplitIcon /></Tooltip>
                            </Typography>
                        </Box>}
                        <Box sx={{
                            display: 'flex', 
