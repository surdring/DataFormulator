...
export const ModelSelectionButton: React.FC<{}> = ({ }) => {
    ...
    const SlotAssignmentSummary: React.FC = () => {
        ...
        return (
            <Box sx={{ mb: 2 }}>
                <Typography variant="body1" sx={{ mb: 2, fontWeight: 'bold' }}>{t('models.assignments.title')}</Typography>
                <Box sx={{ display: 'flex', gap: 1.5, mb: 2, flexWrap: 'wrap' }}>
                    {slotTypes.map(slotType => {
                        ...
                        if (slotType == 'generation') {
                            modelExplanation = t('models.assignments.slot.generation');
                        } else if (slotType == 'hint') {
                            modelExplanation = t('models.assignments.slot.hint');
                        }
                        ...
                        return (
                            ...
                            >
                                <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                                    {t('models.assignments.modelFor').replace('{slot}', slotType)}
                                </Typography>
                                ...
                                <FormControl fullWidth size="small">
                                    <Select
                                        ...
                                        </Box> : t('models.assignments.unknownModel');
                                        }}
                                    >
                                        <MenuItem value="">
                                            <Typography sx={{ color: 'text.secondary', fontSize: '0.875rem' }}>{t('models.assignments.noAssignment')}</Typography>
                                        </MenuItem>
                                        {models.map((model) => (
                ...
                <Typography variant="caption" sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>
                    {t('models.assignments.summaryTip')}
                </Typography>
            </Box>
        );
    };
    ...
    >
        <TableCell align="left">
            <Autocomplete
                ...
                renderInput={(params) => (
                    <TextField
                        {...params}
                        placeholder={t('models.new.provider.placeholder')}
                        slotProps={{
                            input: {
                                ...params.InputProps,
                    ...
                    paper: (props) => {
                        return <Paper {...props}>
                            <Typography sx={{ p: 1, color: 'gray', fontStyle: 'italic', fontSize: '0.75rem' }}>
                                {t('models.new.provider.examples')}
                            </Typography>
                            {props.children}
                        </Paper>
                    }
        ...
        <TableCell align="left" sx={{ minWidth: '180px' }}>
            <TextField fullWidth size="small" type={showKeys ? "text" : "password"} 
                ...
                placeholder={t('models.new.apiKey.placeholder')}
                value={newApiKey}  
        ...
        <TableCell align="left">
            <TextField
                ...
                placeholder={t('models.new.model.placeholder')}
                error={newEndpoint != "" && !newModel}
                slotProps={{
                    input: {
                        style: { fontSize: "0.75rem" },
                        'aria-label': t('models.new.model.placeholder'),
                    }
                }}
            />
        </TableCell>
        <TableCell align="right">
            <TextField size="small" type="text" fullWidth
                placeholder={t('models.new.apiBase.placeholder')}
                slotProps={{
                    input: {
                        style: { fontSize: "0.75rem" }
                    }
        ...
        <TableCell align="right">
            <TextField size="small" type="text" fullWidth
                ...
                autoComplete='off'
                placeholder={t('models.new.apiVersion.placeholder')}
            />
        </TableCell>
        <TableCell align="right">
    ...
    let modelTable = <TableContainer>
        <Table sx={{ minWidth: 600, "& .MuiTableCell-root": { padding: "4px 8px", borderBottom: "none", fontSize: '0.75rem' } }} size="small" >
            <TableHead >
                <TableRow>
                    <TableCell sx={{fontWeight: 'bold', width: '120px'}}>{t('models.table.provider')}</TableCell>
                    <TableCell sx={{fontWeight: 'bold', width: '160px'}}>{t('models.table.apiKey')}</TableCell>
                    <TableCell sx={{fontWeight: 'bold', width: '160px'}} align="left">{t('models.table.model')}</TableCell>
                    <TableCell sx={{fontWeight: 'bold', width: '200px'}} align="left">{t('models.table.apiBase')}</TableCell>
                    <TableCell sx={{fontWeight: 'bold', width: '120px'}} align="left">{t('models.table.apiVersion')}</TableCell>
                    <TableCell sx={{fontWeight: 'bold'}} align="left">{t('models.table.status')}</TableCell>
            ...
            <TableBody>
                {models.map((model) => {
                    ...
                    let message = t('models.assignments.modelReady');
                    if (status == "unknown") {
                        message = t('models.assignments.modelClickToTest');
                    } else if (status == "error") {
                        const rawMessage = testedModels.find(t => t.id == model.id)?.message || 'Unknown error';
                        message = `${t('models.assignments.modelErrorPrefix')}${decodeHtmlEntities(rawMessage)} ${t('models.assignments.modelErrorClickRetest')}`;
                    }
                        ...
                        >
                            ...
                            <TableCell component="th" scope="row" sx={{ borderBottom: borderStyle }}>
                                {model.api_key ? (showKeys ? 
                                    ...
                                    : <Typography variant="body2" sx={{ fontSize: 'inherit', fontFamily: 'monospace', color: 'text.secondary' }}>••••••••••••</Typography>)
                                     : <Typography sx={{color: "text.secondary", fontSize: 'inherit', fontStyle: 'italic'}}>{t('models.table.none')}</Typography>
                                }
                            </TableCell>
                            ...
                            <TableCell sx={{borderBottom: borderStyle}} align="left">
                                <Tooltip title={message}>
                                    ...
                                    >
                                        {status == 'ok' ? t('models.status.button.ready') : status == 'error' ? t('models.status.button.retest') : t('models.status.button.test')}
                                    </Button>
                                </Tooltip>
                            </TableCell>
    ...
    return <>
        <Tooltip title={t('models.button.tooltip')}>
            <Button sx={{fontSize: "inherit", textTransform: "none"}} variant="text" color={notAllSlotsReady ? 'warning' : "primary"} onClick={()=>{setModelDialogOpen(true)}}>
                {notAllSlotsReady ? t('models.button.select') : modelNames}
            </Button>
        </Tooltip>
        <Dialog 
            maxWidth="lg" 
        ...
        >
            <DialogTitle sx={{display: "flex",  alignItems: "center"}}>{t('models.dialog.title')}</DialogTitle>
            <DialogContent >
                <SlotAssignmentSummary />
                <Divider sx={{ my: 2 }} textAlign="left"> 
                    <Typography variant="caption" sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>
                    {t('models.dialog.available')}
                    </Typography>
                </Divider>
                {modelTable}
            </DialogContent>
                ...
                {!serverConfig.DISABLE_DISPLAY_KEYS && (
                    <Button sx={{marginRight: 'auto'}} endIcon={showKeys ? <VisibilityOffIcon /> : <VisibilityIcon />} onClick={()=>{
                        setShowKeys(!showKeys);}}>
                            {showKeys ? t('models.button.hideKeys') : t('models.button.showKeys')}
                    </Button>
                )}
