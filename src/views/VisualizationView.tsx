...
const LazyChartRecBox = React.lazy(() =>
    import('./ChartRecBox').then((m) => ({ default: m.ChartRecBox }))
);

const LazyEncodingShelfThread = React.lazy(() =>
    import('./EncodingShelfThread').then((m) => ({ default: m.EncodingShelfThread }))
);
        ...
        >
            <Box sx={{ p: 2, width: 300 }}>
                <Typography fontSize="small" gutterBottom>
                    {t('vis.sample.adjustLabel')
                        .replace('{current}', String(localSampleSize))
                        .replace('{total}', String(totalSize))}
                </Typography>
                <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Slider
                        ...
                        aria-label={t('vis.sample.sliderLabel')}
                    />
...
export const ChartEditorFC: FC<{}> = function ChartEditorFC({}) {
            ...
            <Typography component="span" sx={{display: 'flex', flexDirection: 'row', alignItems: 'center', whiteSpace: 'nowrap'}} fontSize="inherit">
                ...
                {table.virtual ? (
                    <Tooltip
                        ...
                        title={t('vis.data.virtualTooltip')}
                    >
                        <CloudQueueIcon  sx={{ fontSize: '12px', color: 'text.secondary', mx: 0.5}} />
                    </Tooltip>
    ...
    if (focusedChart.chartType == "Table") {
        chartMessage = t('vis.message.tablePrompt');
    } else if (focusedChart.chartType == "Auto") {
        chartMessage = t('vis.message.autoPrompt');
    } else if (encodingShelfEmpty) {
        chartMessage = t('vis.message.encodingEmpty');
    } else if (chartUnavailable) {
        chartMessage = t('vis.message.chartUnavailable');
    } else if (chartSynthesisInProgress.includes(focusedChart.id)) {
        chartMessage = t('vis.message.synthesisInProgress');
    } else if (table.derive) {
        chartMessage = t('vis.message.aiResultWarning');
    }
            ...
            {(table.virtual || table.rows.length > 1000) && !(chartUnavailable || encodingShelfEmpty) ? (
                <Box sx={{ display: 'flex', flexDirection: "row", margin: "auto", justifyContent: 'center', alignItems: 'center'}}>
                    <Typography component="span" fontSize="small" color="text.secondary" sx={{textAlign:'center'}}>
                        {t('vis.sample.visualizingPrefix')}
                    </Typography>
                    <SampleSizeEditor 
    ...
    let content = [
        ...
        >
            <LazyEncodingShelfThread chartId={focusedChart.id} />
        </React.Suspense>
    ]
