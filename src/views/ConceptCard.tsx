...
let ConceptReApplyButton: FC<{field: FieldItem, 
    ...
    return (
        <>
            <Tooltip
                key="reapply-icon-button"
                title={t('concept.dialog.reapply.tooltip').replace('{table}', focusedTable.displayId || focusedTable.id)}
            >
                <IconButton
                    size="small"
                    key="reapply-icon-button"
            ...
            <Dialog open={applicationDialogOpen} onClose={() => { setApplicationDialogOpen(false) }}>
                <DialogTitle>
                    {t('concept.dialog.reapply.title.prefix')}
                    <Typography color="primary" component="span" sx={{ fontSize: "inherit" }}>{field.name}</Typography>
                    {t('concept.dialog.reapply.title.middle')}
                    <Typography color="primary" component="span" sx={{ fontSize: "inherit"}}>{focusedTable.displayId}</Typography>
                    {t('concept.dialog.reapply.title.suffix')}
                </DialogTitle>
                <DialogContent>
                    <Typography sx={{ fontSize: 12, marginBottom: 1 }}>{t('concept.dialog.reapply.codeLabel')}</Typography>
                    <CodeBox code={codePreview.trim()} language="python" />
                    <Typography sx={{ fontSize: 12, marginBottom: 1 }}>{t('concept.dialog.reapply.previewLabel')}</Typography>
                    <CustomReactTable rows={tableRowsPreview} columnDefs={colDefs} rowsPerPageNum={15} compact={true} maxCellWidth={100} />
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => { setApplicationDialogOpen(false), setTableRowsPreview([]), setCodePreview("") }}>
                        {t('common.cancel')}
                    </Button>
                    <Button onClick={() => { 
...
export const ConceptCard: FC<ConceptCardProps> = function ConceptCard({ field, sx }) {
    ...
    let editOption = field.source == "derived" && (
        <Tooltip key="edit-icon-button" title={t('common.edit') ?? 'edit'}>
    ...
    let deriveOption = (field.source == "derived" || field.source == "original") && (
        <Tooltip key="derive-icon-button" title={t('concept.form.deriveFrom')}>
    ...
    let fieldNameEntry = field.name != "" ? <Typography sx={{
        ...
        : <Typography sx={{ fontSize: 12, marginLeft: "3px", color: "gray", fontStyle: "italic" }}>{t('concept.form.new') ?? 'new concept'}</Typography>;
...
export const DerivedConceptFormV2: FC<ConceptFormProps> = function DerivedConceptFormV2({ concept, handleUpdateConcept, handleDeleteConcept, turnOffEditMode }) {
    ...
    let nameField = (
        <TextField key="name-field" id="name" fullWidth label={t('concept.form.nameLabel')} value={name} sx={{ minWidth: 120, flex: 1, paddingBottom: 1 }}
            slotProps={{
                formHelperText: {
            ...
            multiline
            helperText={conceptShelfItems.some(f => f.name == name && f.id != concept.id) ? t('concept.form.nameExists') : ""}
    ...
    cardTopComponents = [
        ...
        <FormControl fullWidth key="derived-card-control" sx={{ minWidth: 120 }} size="small">
            <InputLabel shrink>{t('concept.form.deriveFrom')}</InputLabel>
            <Select
                labelId="parent-id-select-label"
                id="parent-id-select"
                multiple
                value={transformParentIDs}
                label={t('concept.form.deriveFrom')}
                inputProps={{}}
    ...
    let codeArea = (
        <Box key="code-area-box" sx={{
            ...
            <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                {viewExamples ? <Typography style={{ fontSize: "9px", color: "gray" }}>{t('concept.form.sampleResult')}</Typography> : ""}
                {viewExamples}
                {transformCode ? <Typography style={{ fontSize: "9px", color: "gray" }}>{t('concept.form.codeTitle')}</Typography> : ""}
        ...
        if (status == "ok") {
            ...
            if (results.length > 0) {
                ...
                dispatch(dfActions.addMessages({
                    ...
                    "value": t('messages.concept.candidatesFound')
                        .replace('{name}', name)
                ...
                dispatch(dfActions.addMessages({
                    ...
                    "type": "info",
                    "component": "Field Card",
                    "value": t('messages.concept.candidatesEmpty')
                        .replace('{name}', name)
                        .replace('{count}', String(results.length))
                }));
            }
            ...
            dispatch(dfActions.addMessages({
                ...
                "type": "error",
                "component": "Field Card",
                "value": t('messages.concept.generateFailed')
            }));
        }
    }
                ...
                <ButtonGroup size="small" sx={{ "& button": { textTransform: "none", padding: "2px 4px", marginLeft: "4px" }, flexGrow: 1, justifyContent: "right" }}>
                    ...
                    <Dialog open={codeDialogOpen} onClose={() => { setCodeDialogOpen(false); }}>
                        <DialogTitle sx={{maxWidth: 800}}>
                            {t('concept.form.dialog.title.prefix')}
                            <Typography component="span" variant="h6" color="secondary">{parentConcepts.map(c => c.name).join(", ")}
                            </Typography>
                            {t('concept.form.dialog.title.middle')}
                            <Typography component="span" variant="h6" color="primary">{name}</Typography>
                            {t('concept.form.dialog.title.suffix')}
                        </DialogTitle>
                        <DialogContent>
                            {codeDialogBox}
                                ...
                                <CardContent sx={{display: "flex", flexDirection: "column", flexGrow: 1, overflow: "clip", maxHeight: 800}}>
                                    <Box width="100%" sx={{}}>
                                        <Box className="GroupHeader">
                                          {viewExamples ? <Typography style={{ fontSize: "12px" }}>{t('concept.form.sampleResult.full')}</Typography> : ""}
                                        </Box>
                                    ...
                                    <Box className="GroupHeader" sx={{marginTop: 1}}>
                                        <Typography style={{ fontSize: "12px" }}>{t('concept.form.codeTitle.full')}</Typography>
                                    </Box>
                        ...
                        <DialogActions>
                            <Button onClick={() => { setCodeDialogOpen(false); }}>{t('common.ok')}</Button>
                        </DialogActions>
                    </Dialog>
                    <IconButton size="small"
                        color="primary" aria-label="Delete" component="span"
...
export const PyCodexDialogBox: FC<CodexDialogBoxProps> = function ({ 
    ...
    let formulateButton = <Tooltip title={t('encoding.button.formulate')}>
