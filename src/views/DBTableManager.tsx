...
export const DBTableSelectionDialog: React.FC<{ 
    ...
    const fetchTables = async () => {
        ...
        try {
            ...
            setSystemMessage(t('db.error.fetchTables'), "error");
        }
    };
    ...
    const handleDBUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                setSystemMessage(data.error || t('db.error.uploadTable'), "error");
            }
        } catch (error) {
            setSystemMessage(t('db.error.uploadTable'), "error");
        } finally {
    ...
    const handleDBFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (data.status === 'success') {
                if (data.is_renamed) {
                    setSystemMessage(
                        t('db.message.tableRenamed')
                            .replace('{originalName}', data.original_name)
                            .replace('{tableName}', data.table_name),
                        "warning"
                    );
                } 
                fetchTables();  // Refresh table list
    ...
    const handleDBReset = async () => {
        try {
            ...
            if (data.status === 'success') {
                ...
                setSystemMessage(data.error || t('db.error.resetDatabase'), "error");
            }
        } catch (error) {
            setSystemMessage(t('db.error.resetDatabase'), "error");
        }
    }
    ...
    const handleCleanDerivedViews = async () => {
        ...
        if (unreferencedViews.length > 0) {
            if (confirm(`${t('db.confirm.cleanDerivedViews')}\n${unreferencedViews.map(v => `- ${v.name}`).join("\n")}`)) {
                let deletedViews = [];
                for (let view of unreferencedViews) {
                    try {
                        ...
                        if (data.status === 'success') {
                            ...
                            setSystemMessage(data.error || t('db.error.deleteTable'), "error");
                        }
                    } catch (error) {
                        setSystemMessage(t('db.error.deleteTable'), "error");
                    }
                }
                if (deletedViews.length > 0) {
                    setSystemMessage(
                        t('db.message.cleanedDerivedViews')
                            .replace('{count}', String(deletedViews.length))
                            .replace('{views}', deletedViews.join(', ')),
                        "success"
                    );
                }
                fetchTables();
                ...
                <Button variant="text" size="small" onClick={() => {
                    handleDBDownload(sessionId ?? '')
                        .catch(error => {
                            ...
                            setSystemMessage(t('db.error.downloadDb'), "error");
                        });
        ...
        <Box sx={{ p: 1, display: 'flex', flexDirection: 'column', backgroundColor: alpha(theme.palette.secondary.main, 0.02) }}>
            ...
            {['file upload', ...Object.keys(dataLoaderMetadata ?? {})].map((dataLoaderType) => (
                ...
                >
                    ...
                    >
                        {dataLoaderType === 'file upload' ? t('db.upload.tab.fileUpload') : dataLoaderType}
                    </Typography>
                </Button>
            ))}
        </Box>
    ...
    let tableView = <Box sx={{ flex: 1, width: 880, overflow: 'auto', p: 2 }}>
            ...
            <Box>
                {uploadFileButton(
                    <Typography component="span" fontSize={18} textTransform="none">
                        {isUploading ? t('db.upload.uploading') : t('db.upload.cta')}
                    </Typography>
                )}
            </Box>
        )}
        ...
        {/* Table content */}
        {dbTables.map((dbTable, i) => {
            if (selectedTabKey !== dbTable.name) return null;
            
            const currentTable = dbTable;
            const showingAnalysis = tableAnalysisMap[currentTable.name] !== undefined;
            return (
                <Box key={currentTable.name} sx={{ maxWidth: '100%', overflowX: 'auto', display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <Paper variant="outlined">
                        <Box sx={{ px: 1, display: 'flex', alignItems: 'center', borderBottom: '1px solid rgba(0,0,0,0.1)' }}>
                            <Typography variant="caption" sx={{  }}>
                                {showingAnalysis ? t('db.table.header.columnStatsPrefix') : t('db.table.header.samplePrefix')}
                                ...
                                <Typography component="span" sx={{ml: 1, fontSize: 10, color: "text.secondary"}}>
                                    {t('db.table.header.shape')
                                        .replace('{columns}', String(currentTable.columns.length))
                                        .replace('{rows}', String(currentTable.row_count))}
                                </Typography>
                            </Typography>
                            <Box sx={{ marginLeft: 'auto', display: 'flex', gap: 1 }}>
                                ...
                                >
                                    {showingAnalysis ? t('db.table.button.showSamples') : t('db.table.button.showStats')}
                                </Button>
                                <IconButton 
                                    size="small" 
                                    color="error"
                                    onClick={() => handleDropTable(currentTable.name)}
                                    title={t('db.table.button.drop')}
                                >
                    ...
                    <Button 
                        ...
                        onClick={() => {
                            let selectedTable = dbTables.find(table => table.name === selectedTabKey);
                            if (selectedTable) {
                                handleAddTableToDF(selectedTable);
                                setTableDialogOpen(false);
                            }
                        }}>
                        {t('common.load')}
