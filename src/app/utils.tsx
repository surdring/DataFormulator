...
const CHART_NOMINAL_LIMITS: Record<string, { xy: number; color: number; facet: number }> = {
    'Bar Chart': { xy: 40, color: 16, facet: 12 },
    'Grouped Bar Chart': { xy: 40, color: 16, facet: 12 },
    'Scatter Plot': { xy: 80, color: 16, facet: 12 },
    'Line Chart': { xy: 80, color: 16, facet: 12 },
    'Custom Area': { xy: 80, color: 16, facet: 12 },
    'Heatmap': { xy: 36, color: 0, facet: 12 },
    'Boxplot': { xy: 40, color: 16, facet: 12 },
    default: { xy: 48, color: 24, facet: 16 },
};
...
export const assembleVegaChart = (
    ...
    const nominalLimits = CHART_NOMINAL_LIMITS[chartType] || CHART_NOMINAL_LIMITS['default'];
    ...
    let maxXYToKeep = Math.min(defaultChartWidth * 2 / defaultStepSize, nominalLimits.xy);
    ...
    for (const channel of ['x', 'y', 'column', 'row', 'xOffset', "color"]) {
        ...
        if (encoding?.type === 'nominal') {
            ...
            let baseMax = channel == 'x' || channel == 'y' ? maxXYToKeep : maxFacetNominalValues;
            if (channel === 'color' && nominalLimits.color > 0) {
                baseMax = Math.min(baseMax, nominalLimits.color);
            }
            if ((channel === 'column' || channel === 'row') && nominalLimits.facet > 0) {
                baseMax = Math.min(baseMax, nominalLimits.facet);
            }
            let maxNominalValuesToKeep = baseMax;
            ...
            if (uniqueValues.length > maxNominalValuesToKeep) {
                ...
                if (fieldOriginalType == 'quantitative' || channel == 'color') {
                    valuesToKeep = uniqueValues.sort((a, b) => a - b).slice(0, maxNominalValuesToKeep);
