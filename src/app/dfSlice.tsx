...
export const fetchFieldSemanticType = createAsyncThunk(
    ...
    async (table: DictTable, { getState }) => {
        ...
        let existingTable = state.tables.find(t => t.id === table.id);
        if (existingTable) {
            let hasSemanticTypes = existingTable.names.every(name => {
                let meta = existingTable.metadata && existingTable.metadata[name];
                return meta && meta.semanticType && meta.semanticType !== "";
            });
            if (hasSemanticTypes) {
