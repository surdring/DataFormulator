...
const LazyDataLoadingChatDialog = React.lazy(() =>
    import('../views/DataLoadingChat').then((m) => ({ default: m.DataLoadingChatDialog }))
);
...
export const toolName = t('app.title');
                ...
                <MenuItem onClick={(e) => {}}>
                    <DBTableSelectionDialog buttonElement={
                        <Typography fontSize="inherit" sx={{  }}>
                            <CloudQueueIcon fontSize="inherit" /> {t('data.menu.database')}
                        </Typography>
                    } />
                </MenuItem>
                <MenuItem onClick={(e) => {}}>     
                    <React.Suspense fallback={<Typography fontSize="inherit">{t('common.loading')}</Typography>}>
                        <LazyDataLoadingChatDialog buttonElement={<Typography fontSize="inherit" sx={{  }}>
                ...
                <DialogContent>
                    <Box sx={{ 
                            ...
                            <Box sx={{ flex: 1 }}>
                                <TextField
                                    ...
                                    helperText={defaultChartHeight < 100 || defaultChartHeight > 1000 ? 
                                        t('settings.defaultChartHeight.helper') : ""}
                                />
                            </Box>
                        </Box>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            <Box sx={{ flex: 1 }}>
                                <TextField
                                    label={t('settings.formulateTimeout')}
                                    type="number"
                                    variant="outlined"
                                    ...
                                    helperText={formulateTimeoutSeconds <= 0 || formulateTimeoutSeconds > 3600 ? 
                                        t('settings.formulateTimeout.helper') : ""}
                                    fullWidth
                                />
                                <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            <Box sx={{ flex: 1 }}>
                                <TextField
                                    label={t('settings.maxRepairAttempts')}
                                    type="number"
                                    variant="outlined"
                                    ...
                                    helperText={maxRepairAttempts <= 0 || maxRepairAttempts > 5 ? 
                                        t('settings.maxRepairAttempts.helper') : ""}
                                />
                                <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
...
export const AppFC: FC<AppFCProps> = function AppFC(appProps) {
                ...
                >
                    ...
                    >
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                            <Box component="span">{t('nav.about')}</Box>
                        </Box>
                    </ToggleButton>
                    <ToggleButton 
                        value="app" 
                    ...
                    >
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                            <Box component="span">{t('nav.app')}</Box>
                        </Box>
                    </ToggleButton>
                </ToggleButtonGroup>
                {!isAboutPage && (
                        ...
                        >
                            <ToggleButton value="editor">
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    ...
                                    <Box component="span">{t('viewMode.explore')}</Box>
                                </Box>
                            </ToggleButton>
    ...
    let router = createBrowserRouter([
        {
            ...
            errorElement: <Box sx={{ width: "100%", height: "100%", display: "flex" }}>
                <Typography color="gray" sx={{ margin: "150px auto" }}>{t('error.route.generic')}</Typography>
            </Box>
        }
    ]);
    ...
    let footer = <Box sx={{ zIndex: 0, position: 'absolute', bottom: 0, left: 0, right: 0, color: 'text.secondary', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <Button size="small" color="inherit" 
                sx={{ textTransform: 'none'}} 
                target="_blank" rel="noopener noreferrer" 
                href="Microsoft Azure Website Terms of Use">{t('footer.terms')}</Button>
        <Divider orientation="vertical" variant="middle" flexItem sx={{ mx: 1 }} />
        <Button size="small" color="inherit" 
                sx={{ textTransform: 'none'}} 
                target="_blank" rel="noopener noreferrer" 
